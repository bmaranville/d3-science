(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    global.profileInteractor = mod.exports;
  }
  ...
  function profileInteractor(state, x, y) {
    ...
    function data_to_pairs(data, column) {
      ...
      return data.map(function (d, i) {
        return [xi += d.thickness, d[column], column];
      });
    }
    ...
    function data_to_arrays(data) {
      ...
      return series.map(function (s) {
        return data_to_pairs(data, s.id);
      });
    }
    ...
    function arrays_to_data(arrays) {
      var columns = series.map(function (s) {
        return s.id;
      });
      var x = 0;
      var output_array = [];
    ...
    var drag_edge = d3.behavior.drag().on("drag", dragmove_edge).on("dragstart", function () {
      _d.event.sourceEvent.stopPropagation();
    });
    ...
    function interactor(selection) {
      ...
      interactor.update = function () {
        var edge_groups_sel = group.selectAll("g.edges").data(data_to_edges(state.profile_data));
        edge_groups_sel.enter().append("g").attr("class", "edges").style("stroke", function (d, i) {
          return (series[i] || {}).color1 || colors[i];
        }).style("stroke-linecap", "round").style("stroke-width", "4px").style("fill", "none");
        edge_groups_sel.exit().remove();

        var corner_groups_sel = group.selectAll("g.corners").data(data_to_arrays(state.profile_data));
        corner_groups_sel.enter().append("g").classed("corners", true).attr("fill", function (d, i) {
          return (series[i] || {}).color1 || colors[i];
        });
        corner_groups_sel.exit().remove();

        corner_groups_sel.each(function (d, i) {
          var corners = d3.select(this).selectAll('.corner').data(d);
          var new_corners = corners.enter().append("circle").classed("corner", true).attr("vertex", function (dd, ii) {
            return ii.toFixed();
          }).attr("r", radius);
          if (!fixed) new_corners.call(drag_corner);
          corners.attr("cx", function (dd) {
            return x(dd[0]);
          }).attr("cy", function (dd) {
            return y(dd[1]);
          }).on("click", function () {
            corner_groups_sel.selectAll("circle.corner").attr("r", radius).classed("selected", false);
        ...
        edge_groups_sel.each(function (d, i) {
          ...
          var new_edges = edges.enter().append("path").classed("edge", true).attr("side", function (dd, ii) {
            return ii.toFixed();
          ...
          d3.select(this).selectAll('.edge').on("dblclick", function (dd, ii) {
            var direction = d3.select(this).attr("direction"),
                ...
                new_row = (0, _jqueryExtend.extend)(true, {}, old_row);
            if (direction == "h") {
          ...
          edges.exit().remove();
          if (draw_extensions) {
            var left_d = (0, _jqueryExtend.extend)(true, [], d[0][0]);
            left_d[0] = x.invert(-10);
            var left_ext = d3.select(this).selectAll('.left.extension').data([[left_d, d[0][0]]]).enter().append("path").classed("left extension", true).attr("direction", "h");
            if (!fixed) left_ext.call(drag_edge);

            var right_d = (0, _jqueryExtend.extend)(true, [], d.slice(-1)[0][0]);
            right_d[0] = x.invert(x.range()[1] + 10);
            var right_ext = d3.select(this).selectAll('.right.extension').data([[right_d, d.slice(-1)[0][0]]]).enter().append("path").classed("right extension", true).attr("direction", "h");
    ...
    function dragmove_corner(d, i) {
      ...
      var new_dx = x.invert(x(0) + _d.event.dx),
          new_dy = y.invert(y(0) + _d.event.dy);
