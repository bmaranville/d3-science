(function (global, factory) {
	...
	if (!d3.hasOwnProperty("id")) d3.id = function () {
  ...
  */
		...
		function update() {
			...
			for (var ii in old_ids) {
				...
				if (old_index != new_index) index_updates[old_index] = new_index;
			}
		...
		function draw_wire() {
			...
			if (src != "cursor") src_pos = get_terminal_pos(src);

			//if the target is not on the cursor
			if (tgt != "cursor") tgt_pos = get_terminal_pos(tgt);
			...
			else connector.attr("d", makeConnector(src_pos, tgt_pos));
		}
		...
		function wirestop() {
			...
			if (active_data.target == 'cursor' || active_data.source == 'cursor') active_wire = false;
		...
		function module(module_data) {
			...
			title = group.append("g").classed("title", true);

			inputs = group.selectAll(".inputs").data(input_terminals).enter().append("g").classed("terminals", true).classed("inputs", true);

			outputs = group.selectAll(".outputs").data(output_terminals).enter().append("g").classed("terminals", true).classed("outputs", true);

			//if a single module
			if (module_name !== "ncnr.refl.combined_module") singleModule(moduleType, module_data, group, input_terminals, output_terminals, padding, title, inputs, outputs, titletext, curX, curY, module_defs, inputEdge, outputEdge);
		...
		function singleModule(moduleType, module_data, group, input_terminals, output_terminals, padding, title, inputs, outputs, titletext, curX, curY, module_defs, inputEdge, outputEdge) {
			...
			titletext = title.append("text").classed("title text", true).text(module_data.title || module_data.module).attr("x", padding + curX).attr("y", padding + curY).attr("dy", "1em");
			...
			inputs.attr("transform", function (d, i) {
				return "translate(-20," + (height * i).toFixed() + ")";
			}).append("text").classed("input label", true).attr("x", curX).attr("y", curY).attr("dy", "1em").style("padding", padding).text(function (d) {
				return d.label;
			});
			...
			if (moduleType === "single") {
				...
				inputs.append("rect").classed("terminal input", true).attr("width", 20).attr("height", height).attr("x", curX).attr("y", curY).attr("wireoffset_x", 0).attr("wireoffset_y", height / 2).attr("terminal_id", function (d) {
					return d.id;
			...
			//if a part of a combined module
					...
					for (var i = 0; i < input_terminals.length; i++) {
						...
						if (inputEdge) {
							...
							inputs.append("rect").classed("terminal input", true).attr("width", 20).attr("height", height).attr("x", curX).attr("y", curY + i * 30).attr("wireoffset_x", 0).attr("wireoffset_y", height / 2).attr("terminal_id", function (d) {
								return d.id;
							}).call(wireaction).append("svg:title").text(function (d) {
								return d.id;
							});
			...
			inputs.append("polygon").classed("terminal input state", true).attr("points", curX + "," + curY + " 20," + (height / 2).toFixed() + " 0," + height.toFixed());

			//add output elements to group	
			outputs.append("text").classed("output label", true).attr("x", curX).attr("y", curY).style("dy", "1em").style("padding", padding).text(function (d) {
				return d.label;
			});
			...
			if (moduleType === "single") {
				...
				outputs.append("rect").attr("transform", function (d, i) {
					return "translate(" + width.toFixed() + "," + (height * i).toFixed() + ")";
				}).classed("terminal output", true).attr("width", 20).attr("height", height).attr("x", curX).attr("y", curY).attr("wireoffset_x", 20).attr("wireoffset_y", height / 2).attr("terminal_id", function (d) {
					return d.id;
				}).call(wireaction).append("svg:title").text(function (d) {
					return d.id;
				});
			...
			//if a part of a combined module
					...
					for (var i = 0; i < output_terminals.length; i++) {
						...
						if (outputEdge) {
							...
							outputs.append("rect").classed("terminal output", true).attr("width", 20).attr("height", height).attr("x", curX + width).attr("y", curY + i * 30).attr("wireoffset_x", 0).attr("wireoffset_y", height / 2).attr("terminal_id", function (d) {
								return d.id;
							}).call(wireaction).append("svg:title").text(module_data.id); //.text(function(d) { return d.id; });
						}
			...
			outputs.append("polygon").classed("terminal input state", true).attr("points", curX + "," + curY + " 20," + (height / 2).toFixed() + " 0," + height.toFixed());

			//add title border last to make sure it's on top
			title.append("rect").classed("title border", true).attr("width", width).attr("height", height).attr("x", curX).attr("y", curY);
		...
		function combinedModule(moduleType, group, padding, title, inputs, outputs, titletext, curX, curY, module_defs, inputEdge, outputEdge) {
			...
			for (var i = 0; i < wires.length; i++) {
				...
				if (edgeSources.indexOf(curSource) === -1) edgeSources.push(curSource);

				//if target source is not there before
				if (edgeTargets.indexOf(curTarget) === -1) edgeTargets.push(curTarget);
		...
		function getNumUsedInputs(curModule, mods, module_defs, curWires, addedModules) {
			...
			for (var i = 0; i < curWires.length; i++) {
				...
				for (var j = 0; j < addedModules.length; j++) {
					...
					if (addedModules[j] === curSource) numUsedInputs++;
				}
			}
		...
		function addModule(moduleType, group, padding, title, inputs, outputs, titletext, curX, curY, module_defs, inputEdge, outputEdge, curModule, wires, addedModules, mods, loc) {
			...
			while (curWires.length !== 0 && addedModules.indexOf(curModule) === -1 && addedModules.length < mods.length - 1) {
				...
				if (curWires.length > 1) curY -= (curWires.length - 1) * 30;
		...
		function getSourceWires(wires, source) {
			...
			for (var i = 0; i < wires.length; i++) {
				...
				//if found the wire
				if (String(curWire.source).split(",")[0] === source) sourceWires.push(curWire);
			}
		...
		function getTargetWires(wires, target) {
			...
			for (var i = 0; i < wires.length; i++) {
				...
				//if found the wire
				if (String(curWire.target).split(",")[0] === target) targetWires.push(curWire);
			}
