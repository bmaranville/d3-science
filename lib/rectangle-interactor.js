(function (global, factory) {
  ...
  function rectangleInteractor(state, x, y) {
    ...
    function dragmove_center() {
      state.xmin = x.invert(x(state.xmin) + _d.event.dx);
      state.xmax = x.invert(x(state.xmax) + _d.event.dx);
      state.ymin = y.invert(y(state.ymin) + _d.event.dy);
      state.ymax = y.invert(y(state.ymax) + _d.event.dy);
      interactor.update();
    }
    ...
    function dragmove_corner(d) {
      ...
      switch (vertex) {
        ...
        case 3:
          ...
          break;
        default:
          console.log("default", _d.event, d3.select(this));
      }
      interactor.update();
    }
