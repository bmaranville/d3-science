(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    global.dataflowEditor = mod.exports;
  }
  ...
  function editor(options) {
    ...
    var options = (0, _jqueryExtend.extend)(true, default_options, options);
    var svg, container;
    ...
    function update() {
      ...
      var key_fn = function key_fn(d) {
        if (d.module_id == undefined) {
          var id = d3.id();
          Object.defineProperty(d, "module_id", { enumerable: false, get: function get() {
              return id;
            } });
        }
        return d.module_id;
      ...
      var exposed_inputs_update = svg.selectAll("g.exposed-terminals.inputs").data(function (d) {
        return d.inputs || [];
      });
      exposed_inputs_update.enter().append(exposed_input);
      exposed_inputs_update.exit().remove();

      var exposed_outputs_update = svg.selectAll("g.exposed-terminals.outputs").data(function (d) {
        return d.outputs || [];
      });
    ...
    function get_terminal_pos(term_id) {
      var parent = term_id[0] == "-1" ? svg.selectAll('.exposed-terminals') : svg.selectAll('g.module[index="' + term_id[0] + '"]');
    ...
    function Editor(selection, datum) {
      ...
      svg = selection.append("svg").datum(datum).classed("editor", true);
    ...
    Editor.export = function () {
      return (0, _jqueryExtend.extend)(true, {}, svg.datum());
    };

    Editor.import = function (datum, copyfirst) {
      // first the modules...
      if (copyfirst) {
        var datum = (0, _jqueryExtend.extend)(true, {}, datum);
      }
    ...
    function wirestart() {
      ...
      if (_d.event.sourceEvent.button > 0 || !d3.select(parent_el).classed("wireable")) {
    ...
    function wirestop() {
      ...
      if (!active_data || active_data.target == 'cursor' || active_data.source == 'cursor') {
        ...
        if (active_data.target[0] < 0) {
          ...
          var to_change = svg.datum().outputs.filter(function (f) {
            return f.id == active_data.target[1];
          })[0];
          to_change.target = active_data.source;
          active_wire = false;
        }
        if (active_data.source[0] < 0) {
          var to_change = svg.datum().inputs.filter(function (f) {
            return f.id == active_data.source[1];
          })[0];
      ...
      update();
      svg.selectAll(".terminal").classed("active-wiring", false).on("mouseenter", null).on("mouseleave", null);
      active_wire = false;
    }
    ...
    function module(module_data) {
      ...
      var input_terminals = (0, _jqueryExtend.extend)(true, [], module_def.inputs || []);
      var output_terminals = (0, _jqueryExtend.extend)(true, [], module_def.outputs || []);
      if (is_embedded_module) {
        input_terminals = input_terminals.map(function (t) {
          return resolve_terminal(module_def, 'inputs', t);
        });
        output_terminals = output_terminals.map(function (t) {
          return resolve_terminal(module_def, 'outputs', t);
        });
      }
      ...
      function dragmove() {
        ...
        var dx = Math.round(_d.event.x / grid_spacing) * grid_spacing - module_data.x;
        var dy = Math.round(_d.event.y / grid_spacing) * grid_spacing - module_data.y;
        module_data.x += dx;
        module_data.y += dy;
        group.attr("transform", "translate(" + module_data.x.toFixed() + "," + module_data.y.toFixed() + ")");
        dispatch.drag_module.call(this, module_data, dx, dy);
        draw_wires();
      }

      // create and append module HTML element:
      group = d3.select(this).append("g").datum(module_data).classed("module draggable wireable", true).style("cursor", "move").attr("transform", "translate(" + module_data.x.toFixed() + "," + module_data.y.toFixed() + ")").attr("x-origin", module_data.x.toFixed()).attr("y-origin", module_data.y.toFixed());
      ...
      var titletext = title.append("text").classed("title text", true).text(function (d) {
        return d.title || d.module;
      }).attr("x", padding).attr("y", padding).attr("dy", "1em");
      ...
      var inputs = group.selectAll(".inputs").data(input_terminals).enter().append("g").classed("terminals", true).classed("inputs", true).attr("transform", function (d, i) {
        return "translate(-20," + (height * i).toFixed() + ")";
      });

      var outputs = group.selectAll(".outputs").data(output_terminals).enter().append("g").classed("terminals", true).classed("outputs", true).attr("transform", function (d, i) {
      ...
      inputs.append("rect").classed("terminal input", true).attr("width", 20).attr("height", height).attr("wireoffset_x", 0).attr("wireoffset_y", height / 2).attr("terminal_id", function (d) {
        return d.id;
      }).call(wireaction).append("svg:title").text(function (d) {
        return d.id;
      });

      inputs.append("polygon").classed("terminal input state", true).attr("points", "0,0 20," + (height / 2).toFixed() + " 0," + height.toFixed());
      ...
      outputs.append("rect").classed("terminal output", true).attr("width", 20).attr("height", height).attr("wireoffset_x", 20).attr("wireoffset_y", height / 2).attr("terminal_id", function (d) {
        return d.id;
      }).call(wireaction).append("svg:title").text(function (d) {
        return d.id;
      });

      outputs.append("polygon").classed("terminal input state", true).attr("points", "0,0 20," + (height / 2).toFixed() + " 0," + height.toFixed());

      //add title border last to make sure it's on top
      title.append("rect").classed("title border", true).attr("width", width).attr("height", height).attr("x", 0).attr("y", 0);
    ...
    function wire(wire_data) {
      var connector = d3.select(this).append("path").classed("wire", true);
      return connector.node();
    }

    function exposed_wire(wire_data) {
      var connector = d3.select(this).append("path").classed("exposed-wire", true);
      return connector.node();
    }
    ...
    function exposed_input(input_data, i) {
      ...
      var exposed_group = svg.append("g").classed("exposed-terminals wireable inputs", true).datum(input_data).attr("index", "-1").attr("transform", "translate(0, " + (height * i).toFixed() + ")");

      exposed_group.append('g').append('rect').classed("terminal", true).classed("output", true).classed("exposed", true).attr("width", width).attr("height", height).attr("wireoffset_x", width).attr("wireoffset_y", height / 2).attr("terminal_id", function (d, i) {
        return d.id;
      }).call(wireaction).append("svg:title").text(function (d) {
        return d.id;
      });
    ...
    function exposed_output(output_data, i) {
      ...
      var exposed_group = svg.append("g").classed("exposed-terminals wireable outputs", true).datum(output_data).attr("index", "-1").attr("transform", "translate(" + (svg_width - width).toFixed() + "," + (height * i).toFixed() + ")");

      exposed_group.append('g').append('rect').classed("terminal", true).classed("input", true).classed("exposed", true).attr("width", width).attr("height", height).attr("wireoffset_x", 0).attr("wireoffset_y", height / 2).attr("terminal_id", function (d, i) {
        return d.id;
      }).call(wireaction).append("svg:title").text(function (d) {
        return d.id;
      });
    ...
    function resolve_terminal(module_def, side, terminal) {
      ...
      var target_terminal = target_module_def[side].filter(function (f) {
        return f.id == target_terminal_id;
      })[0];
      var resolved_terminal = is_target_embedded ? resolve_terminal(target_module_def, side, target_terminal) : target_terminal;
      // make a copy...
      resolved_terminal = (0, _jqueryExtend.extend)(true, {}, resolved_terminal);
